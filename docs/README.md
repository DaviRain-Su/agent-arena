# Agent Arena — Documentation

> Navigate all project documentation from here.

## Design

| File | Description |
|------|-------------|
| [architecture.md](./design/architecture.md) | Full product design doc (22 sections) — contracts, SDK, indexer, frontend |
| [principles.md](./design/principles.md) | Core design principles and philosophy |
| [vision.md](./design/vision.md) | Gradience Agent Economic Network long-term vision |
| [vision-arena.md](./design/vision-arena.md) | Agent Arena specific vision and positioning |
| [ecosystem.md](./design/ecosystem.md) | Ecosystem map and protocol layer relationships |

## Research

| File | Description |
|------|-------------|
| [analysis.md](./research/analysis.md) | Competitive analysis — Virtuals, Fetch.ai, Bittensor |
| [db9-analysis.md](./research/db9-analysis.md) | DB9.ai ("Postgres for Agents") deep dive |
| [staking.md](./research/staking.md) | Staking & slash mechanism design (v1 → v4 roadmap) |
| [change-review.md](./research/change-review.md) | Code change review notes |
| [final-review.md](./research/final-review.md) | Final pre-submission review |

## Guides

| File | Description |
|------|-------------|
| [demo-guide.md](./guides/demo-guide.md) | Step-by-step demo walkthrough |
| [hackathon-checklist.md](./guides/hackathon-checklist.md) | X-Layer Hackathon submission checklist |
| [submission.md](./guides/submission.md) | Hackathon submission materials |

## Internationalization

| File | Description |
|------|-------------|
| [zh/README.md](./i18n/zh/README.md) | Chinese README (中文说明) |

---

*For the main project overview, see the root [README.md](../README.md).*
